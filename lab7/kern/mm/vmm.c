...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
...
#endif
   ptep = get_pte(mm->pgdir, addr, 1);//get or create a new one
   ...
   if(ptep != NULL && *ptep == 0){
	...
	if(swap_in(mm, addr, &tempPage))
	    goto failed;
	page_insert(mm->pgdir, tempPage, addr, perm);
	swap_map_swappable(mm, addr, tempPage, 1);
   }
   ret = 0;
failed:
    return ret;
